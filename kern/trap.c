...
//	cprintf("fault_va Round Down: %x\n",fault_va);
	...
	if(curenv->SecondList.size != 0 && curenv->SecondListSize != 0)
	{
		...
		LIST_FOREACH(element, &(curenv->SecondList))
		{
			...
			{
		 		//remove the page from the second list
				LIST_REMOVE(&(curenv->SecondList),element);

				struct WorkingSetElement *TailofActiveList =  LIST_LAST(&(curenv->ActiveList));

				//remove the tail of the active list
				LIST_REMOVE(&(curenv->ActiveList),TailofActiveList);

				//add to the head of the second list
				LIST_INSERT_HEAD(&(curenv->SecondList),TailofActiveList);

				//set persmession present bit 0
				pt_set_page_permissions(curenv,TailofActiveList->virtual_address,0,PERM_PRESENT);
	...
	struct Frame_Info * frame = get_frame_info(curenv->env_page_directory, (void*)fault_va, &pagetable);
	allocate_frame(&frame);
	map_frame(curenv->env_page_directory, frame, (void*)fault_va, PERM_USER|PERM_WRITEABLE);
...
//			print_page_working_set_or_LRUlists(curenv);
	...
	//remove the element from working set
	LIST_REMOVE(&(curenv->PageWorkingSetList),WS_element);

	//set the present bit 1
	pt_set_page_permissions(curenv,WS_element->virtual_address,PERM_PRESENT,0);

	//add the element to the active list
	LIST_INSERT_HEAD(&(curenv->ActiveList),WS_element);
